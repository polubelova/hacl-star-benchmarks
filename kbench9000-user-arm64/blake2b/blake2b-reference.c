...
static const uint64_t ref_blake2b_IV[8] =
{
...
/*
static const uint8_t ref_blake2b_sigma[12][16] =
{
...
static void ref_blake2b_set_lastnode( ref_blake2b_state *S )
{
  S->f[1] = (uint64_t)-1;
}

/* Some helper functions, not necessarily useful */
static int ref_blake2b_is_lastblock( const ref_blake2b_state *S )
{
  return S->f[0] != 0;
}

static void ref_blake2b_set_lastblock( ref_blake2b_state *S )
{
  if( S->last_node ) ref_blake2b_set_lastnode( S );

  S->f[0] = (uint64_t)-1;
}

static void ref_blake2b_increment_counter( ref_blake2b_state *S, const uint64_t inc )
{
  S->t[0] += inc;
  S->t[1] += ( S->t[0] < inc );
}

static void ref_blake2b_init0( ref_blake2b_state *S )
{
  size_t i;
  memset( S, 0, sizeof( ref_blake2b_state ) );

  for( i = 0; i < 8; ++i ) S->h[i] = ref_blake2b_IV[i];
}
...
static void ref_blake2b_compress( ref_blake2b_state *S, const uint8_t block[REF_BLAKE2B_BLOCKBYTES] )
{
  ...
  row3l = vld1q_u64(&ref_blake2b_IV[0]);
  row3h = vld1q_u64(&ref_blake2b_IV[2]);
  row4l = veorq_u64(vld1q_u64(&ref_blake2b_IV[4]), vld1q_u64(&S->t[0]));
  row4h = veorq_u64(vld1q_u64(&ref_blake2b_IV[6]), vld1q_u64(&S->f[0]));
...
int ref_blake2b_update( ref_blake2b_state *S, const void *pin, size_t inlen )
{
  ...
  if( inlen > 0 )
  {
    ...
    if( inlen > fill )
    {
      ...
      ref_blake2b_increment_counter( S, REF_BLAKE2B_BLOCKBYTES );
      ref_blake2b_compress( S, S->buf ); /* Compress */
      in += fill; inlen -= fill;
      while(inlen > REF_BLAKE2B_BLOCKBYTES) {
        ref_blake2b_increment_counter(S, REF_BLAKE2B_BLOCKBYTES);
        ref_blake2b_compress( S, in );
...
int ref_blake2b_final( ref_blake2b_state *S, void *out, size_t outlen )
{
  ...
  if( ref_blake2b_is_lastblock( S ) )
    return -1;

  ref_blake2b_increment_counter( S, S->buflen );
  ref_blake2b_set_lastblock( S );
  memset( S->buf + S->buflen, 0, REF_BLAKE2B_BLOCKBYTES - S->buflen ); /* Padding */
  ref_blake2b_compress( S, S->buf );
...
#if defined(REF_BLAKE2B_SELFTEST)
#include <string.h>
...
int main( void )
{
  ...
  for( i = 0; i < BLAKE2_KAT_LENGTH; ++i )
  {
    ...
    if( 0 != memcmp( hash, ref_blake2b_keyed_kat[i], REF_BLAKE2B_OUTBYTES ) )
    {
      goto fail;
    }
  }
...
{
  ref_blake2b(output, nn, d, ll, k, kk);
}

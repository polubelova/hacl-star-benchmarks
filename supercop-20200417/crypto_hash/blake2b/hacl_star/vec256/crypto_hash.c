...
extern void Hacl_Blake2b_256_blake2b(uint32_t nn, uint8_t *output, uint32_t ll, uint8_t *d, uint32_t kk, uint8_t *k);
...
int crypto_hash(unsigned char *hash, const unsigned char *in, unsigned long long inlen) {
  Hacl_Blake2b_256_blake2b(64, hash, inlen, in, 0, NULL);
  return 0;
}

...
void sha224_ ## name(u8* input_data,  u32 len, u8 * output); \
static inline int name(size_t len) \
{ \
	sha224_ ## name(input_data, len, dummy_out); \
}
...
#define test_it(name, before, after) do { \
	memset(out, __LINE__, vectors_sha224[i].expected_len); \
	before; \
	sha224_ ## name(vectors_sha224[i].input, vectors_sha224[i].input_len, out); \
	after; \
	if (memcmp(out, vectors_sha224[i].expected, vectors_sha224[i].expected_len)) { \
